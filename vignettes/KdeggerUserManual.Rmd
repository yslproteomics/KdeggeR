...
### Spectronaut 19 example, use filepath
  ...
  - The files are provided in the data_tsv directory via the GitHub repository. 
...
```{r filter valid values skip time point 1, eval = TRUE}
...
```{r filter valid values, eval = TRUE}
...
```{r filter monotone trend skip time point 1, eval = TRUE}
...
```{r filter monotone trend, eval = TRUE}
# example code, does not modify the pSILAC object
...
```{r filter linear regression, eval = TRUE}
...
```{r fit precursor RIA k_loss, eval = TRUE}
...
```{r fit precursor NLI k_loss, eval = TRUE}
...
```{r fit precursor HOL k_loss, eval = TRUE}
...
```{r calculate protein k_loss, eval = TRUE}
...
```{r calculate protein k_loss and export table, eval = TRUE}
prot_kloss <- calcProteinsKloss(pSILAC_object, method = "complement", 
                       ag.metric = "mean", 
                       ag.weights = "both", 
                       ncores = 1, 
...
The `calcAllRates()` function is a wrapper function that runs precursor k_loss calculation using the RIA, ln(H/L +1), and NLI data. 

Then it aggregates the precursor-level estimates into protein k_loss estimates using the selected method. This function can be used 
