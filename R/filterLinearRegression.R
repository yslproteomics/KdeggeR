...
filterLinearRegression_v2 <- function(o, skip_time_point = 1,
  ...
  # Sample-specific time vectors (critical for unbalanced designs!)
  time_points_list <- split(o$design$time, samples)
  
  # Split hol and ria data by sample (columns grouped by sample)
  split_hol <- lapply(split.default(hol_data, samples), as.data.frame)
  split_ria <- lapply(split.default(ria_data, samples), as.data.frame)
  
  # Sanity check: times and columns must match per sample
  stopifnot(
    length(split_hol) == length(time_points_list),
    all(names(split_hol) == names(time_points_list))
  )
  
  message(paste(Sys.time(), "Performing the linear fit of the ln(H/L) data...", sep = " "))
  
  # Fit linear model per sample using that sample's time vector
  split_hol_fit <- vector("list", length(split_hol))
  names(split_hol_fit) <- names(split_hol)
  
  for (i in seq_along(split_hol)) {
    tp_i <- as.numeric(time_points_list[[i]])
    hol_i <- split_hol[[i]]
    
    if (length(tp_i) != ncol(hol_i)) {
      stop("filterLinearRegression_v2: length of time points (", length(tp_i),
           ") does not match number of columns (", ncol(hol_i),
           ") for sample '", names(split_hol)[i], "'.")
    }
    
    split_hol_fit[[i]] <- fitLinearModel(
      hol_i,
  ...
  for (i in seq_along(split_hol_fit)) {
    ...
    # IDs to remove from the first time point
    remove_ids <- data_fit %>%
      dplyr::filter(time_point1 == TRUE,
                    R2_full < R2_cutoff,
                    grubbs_pval < p_cutoff) %>%
      tibble::rownames_to_column("id") %>%
      dplyr::pull(id)
    
    message(sprintf("Data points removed from tp1: %d, sample: %s",
                    length(remove_ids), names(split_hol_fit)[i]))
