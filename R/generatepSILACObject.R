...
generatepSILACObject <- function(dataset, design, inputDataType = "spectronaut", requant="remove", aggregate.replicates=NA, filterPeptides=T, ncores=1, imputeMethod="normD1", noiseCutoff = 8){
  ...
  ## Detect duplicate timepoints within sample
  sample_time <- paste(design$sample, design$time)
  has_dup     <- any(duplicated(sample_time))
  
  ## If replicates exist but user did not specify aggregation → error
  if (is.na(aggregate.replicates) && has_dup) {
    stop(
      "Replicates detected: some (sample, time) combinations appear more than once.\n",
      "Either:\n",
      "  - set 'aggregate.replicates' (e.g. mean, median), or\n",
      "  - modify the design so each (sample, time) combination is unique ",
      "    (e.g. treat each replicate as a separate sample)."
    )
  }
  ...
  # This part was changed to enable single timepoint replicates
  if (!is.na(aggregate.replicates)) {
    ...
    if (length(unique(nnames)) == length(nnames)) {
      ...
      ## collapse design to one row per (sample, time)
      design <- aggregate(design, by = list(name = nnames),
                          FUN = function(x) x[[1]])
      o <- order(design$sample, design$time)
      design <- design[o, ]
      row.names(design) <- design$name
      
      unique_names <- row.names(design)
      
      if (is.null(ncores)) {
        ncores <- parallel::detectCores() - 1
      } else if (ncores > 1) {
        # just load parallel; nothing else yet
      }
      
      agg_fun <- function(nm) {
        cols <- which(nnames == nm)
        if (length(cols) == 1L) {
          
          warning(
            paste0(
              "Replicate aggregation: Only one measurement found at timepoint '", nm,
              "'. This is not a true replicate.\n",
              "The value will be carried through as-is. If your replicates have staggered ",
              "or non-overlapping timepoints (e.g., Rep1: 6–72h, Rep2: 12–96h), consider:\n",
              "  - disabling replicate aggregation, OR\n",
              "  - interpolating replicates onto a common time grid, OR\n",
              "  - computing k_loss per replicate rather than averaging."
            ),
            call. = FALSE
          )
          
          # no replicate → just return that column
          e[, cols]
        } else {
          # true replicates → aggregate across columns, per protein
          apply(e[, cols, drop = FALSE], 1,
                aggregate.replicates, na.rm = TRUE)
        }
      }
      
      if (ncores > 1) {
        cl <- parallel::makeCluster(ncores)
        parallel::clusterExport(cl, c("nnames", "e", "aggregate.replicates"),
                                environment())
        e_list <- parallel::parLapply(cl, unique_names, agg_fun)
        parallel::stopCluster(cl)
      } else {
        e_list <- lapply(unique_names, agg_fun)
      }
      
      # bind aggregated columns in the new time-order
      e <- do.call(cbind, e_list)
      colnames(e) <- unique_names
    }
  }
...
#' @title Generate pSILAC class object, outdated
#'
#' @description Creates a pSILAC object. Needs a design table and precursor-level report from an DIA-MS data processing software such as
#' Spectronaut, DIA-NN, and Fragpipe. 
#' These can be provided as path or as data loaded in R environment. 
...
#'
#' @keywords internal
.generatepSILACObject_v0 <- function(dataset, design, inputDataType = "spectronaut", requant="remove", aggregate.replicates=NA, filterPeptides=T, ncores=1, imputeMethod="normD1", noiseCutoff = 8){
