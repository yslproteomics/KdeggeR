...
#'
#' @description
#' This function calculates protein degradation rates based on the protein-level
#' `kloss` values from a pSILAC object. It supports two types of calculations:
#' estimating theoretical degradation rates using a specified quantile (`kperc`)
...
#'
#' @return
#' A modified pSILAC object with an added `protein.kdeg` matrix, representing the calculated
#' degradation rates for each protein. Any kdeg values <= 0 are replaced by NA before storing.
#'
#' @importFrom dplyr select ends_with filter pull slice rename_with any_of
#' @export
calcKdeg <- function(o, rate_df = NULL, type = "kperc", perc_neg = 0.01) {
  ...
  if (is.null(rate_df) && type == "kperc") {
    ...
    stop("Invalid 'type'. Choose 'kperc' or 'kcd'.")
  }
  
  # Rename columns from *.kloss -> *.kdeg
  prot_kdeg <- prot_kdeg %>%
    dplyr::rename_with(cols = dplyr::everything(), ~ gsub(".kloss", ".kdeg", ., fixed = TRUE))
  
  # Replace non-positive kdeg with NA (before storing in the object)
  mat <- as.matrix(prot_kdeg)
  n_replace <- sum(mat <= 0, na.rm = TRUE)
  if (n_replace > 0L) {
    message(paste(Sys.time(), "Replacing", n_replace, "kdeg values <= 0 with NA."))
    mat[mat <= 0] <- NA_real_
    prot_kdeg <- as.data.frame(mat, stringsAsFactors = FALSE)
    colnames(prot_kdeg) <- colnames(o$protein.kloss %>%
                                      dplyr::select(-dplyr::ends_with(".stderr"), -dplyr::any_of("source"))) %>%
      gsub(".kloss", ".kdeg", ., fixed = TRUE)
    rownames(prot_kdeg) <- rownames(o$protein.kloss %>%
                                      dplyr::select(-dplyr::ends_with(".stderr"), -dplyr::any_of("source")))
  }
