...
calcProteinsKloss <- function(o, method = "combined", ag.metric = "mean", ag.weights = "both",
  ...
  if (ag.metric == "remodel" & method != "complement") {
    stop("Protein k_loss remodeling not yet enabled.")
    return(modelProteinsKloss(
      o, method = method, ag.weights = ag.weights, in.all = in.all, removeOutliers = removeOutliers,
      ncores = ncores, tryRobust = tryRobust, returnKlossTableOnly = returnKlossTableOnly, returnSD = returnSD
    ))
  }
  ...
  if (method %in% c("combined", "complement")) {
    ...
    # Calculate protein k_loss using both RIA and NLI-based methods
    k1 <- calcProteinsKloss(
      o, method = "RIA", ag.metric = ag.metric, ag.weights = ag.weights, unique.weights = unique.weights,
      in.all = in.all, removeOutliers = removeOutliers, ncores = ncores, tryRobust = tryRobust,
      returnKlossTableOnly = TRUE, returnSD = method == "combined"
    )
    k2 <- calcProteinsKloss(
      o, method = "NLI", ag.metric = ag.metric, ag.weights = ag.weights, unique.weights = unique.weights,
      in.all = in.all, removeOutliers = removeOutliers, ncores = ncores, tryRobust = tryRobust,
      returnKlossTableOnly = TRUE, returnSD = method == "combined"
    )
    ...
    if (method == "complement") {
      ...
      # Filter out incomplete or invalid RIA results and add complementary NLI results
      k1 <- k1[which(!apply(k1, 1, FUN = function(k) { any(is.na(k) | k < 0) })), , drop = FALSE]
      k2 <- k2[which(!(row.names(k2) %in% row.names(k1))), , drop = FALSE]
      ...
      # Indices for paired columns (kloss in odd positions; stderr in even positions)
      idx_k1_kloss <- 2 * seq_len(ncol(k1) / 2) - 1
      idx_k1_sd    <- 2 * seq_len(ncol(k1) / 2)
      idx_k2_kloss <- 2 * seq_len(ncol(k2) / 2) - 1
      idx_k2_sd    <- 2 * seq_len(ncol(k2) / 2)
      
      # Filter k1 based on kloss validity (protect against vectorization)
      k1 <- k1[
        which(!apply(k1[, idx_k1_kloss, drop = FALSE], 1, FUN = function(k) { any(is.na(k) | k < 0) })),
        , drop = FALSE
      ]
      
      # Complementary rows from k2 (protect against 1-col drop)
      k2comp <- k2[which(!(row.names(k2) %in% row.names(k1))), idx_k2_kloss, drop = FALSE]
      
      # Align k2 rows to k1 (protect against dimension drop)
      k2 <- k2[row.names(k1), , drop = FALSE]
      
      # Compute stability metrics (protect against 1-col drop into vector)
      msd1 <- apply(k1[, idx_k1_sd, drop = FALSE], 1, FUN = function(x) {
        abs(mean(x[2 * seq_len(length(x) / 2)], na.rm = TRUE) /
              median(x[2 * seq_len(length(x) / 2) - 1], na.rm = TRUE))
      })
      msd2 <- apply(k2[, idx_k2_sd, drop = FALSE], 1, FUN = function(x) {
        abs(mean(x[2 * seq_len(length(x) / 2)], na.rm = TRUE) /
              median(x[2 * seq_len(length(x) / 2) - 1], na.rm = TRUE))
      })
      
      # Keep only kloss columns (protect against 1-col drop into vector)
      k1 <- k1[, idx_k1_kloss, drop = FALSE]
      k2 <- k2[, idx_k2_kloss, drop = FALSE]
      
      # Substitute less stable RIA-based estimates with NLI-based estimates when applicable
      sel <- which(msd1 > msd2)
      if (length(sel) > 0) {
        k1[sel, ] <- k2[sel, , drop = FALSE]
      }
    ...
    # Record method information
    o$info$protk.method <- paste(
      "Protein-level k_loss calculated through the", method, "method with", ag.metric, "of peptides",
      ifelse(ag.metric == "mean" & !is.null(ag.weights),
             paste("weighted by", ifelse(ag.weights == "both", "nbpoints/variance", ag.weights)), "")
    )
    o$info$protk.call <- match.call()
  ...
  # Initialize k_loss data frame with appropriate columns for k_loss and SD if requested
  if (returnSD) {
    o$protein.kloss <- as.data.frame(
      matrix(NA, nrow = length(unique(proteins)), ncol = 2 * length(unique(o$design$sample))),
      row.names = unique(proteins)
    )
    colnames(o$protein.kloss) <- paste(
      rep(unique(o$design$sample), each = 2),
      rep(c("kloss", "stderr"), length(unique(o$design$sample))),
      sep = "."
    )
  } else {
    o$protein.kloss <- as.data.frame(
      matrix(NA, nrow = length(unique(proteins)), ncol = length(unique(o$design$sample))),
      row.names = unique(proteins)
    )
    colnames(o$protein.kloss) <- paste(unique(o$design$sample), "kloss", sep = ".")
  }
  ...
  for (p in unique(proteins)) {
    ...
    # Aggregate peptide-level k_loss for the current protein (protect against 1-row drop)
    ee <- e[which(proteins == p), , drop = FALSE]
    o$protein.kloss[p, ] <- aggregateKloss(
      ee, unique(o$design$sample), ag.metric, ag.weights, unique.weights, in.all, removeOutliers,
      returnSD = returnSD
    )
  }
  cat("\n")
  ...
  # Remove rows with all NA values
  o$protein.kloss <- o$protein.kloss[
    which(apply(o$protein.kloss, 1, FUN = function(y) { !all(is.na(y)) })),
    , drop = FALSE
  ]
